...
class RSIMeanReversionStrategy(BaseStrategy):
    metadata = StrategyMetadata(
        ...
        description="Bidirectional RSI mean reversion with trend confirmation.",
        default_parameters={
            "rsi_window": 14,
            "oversold": 30,
            "overbought": 70,
    ...
    def generate_signals(self, market_data: pd.DataFrame) -> pd.DataFrame:
        ...
        long_entry = (df["rsi"] <= float(self.params["oversold"])) & (df["close"] >= df["trend_ema"])
        short_entry = (df["rsi"] >= float(self.params["overbought"])) & (df["close"] <= df["trend_ema"])
        exit_signal = (
            (df["rsi"] >= float(self.params["exit_rsi"]))
            | (df["rsi"] <= float(self.params["cover_rsi"]))
            | ((df["close"] - df["trend_ema"]).abs() / df["close"] < 0.001)
        )
        df["signal"] = 0
